...
type op_t = Plus | Minus | Times | Devide | Equal | Less | NotEqual | OrLess
...
let rec to_string exp = match exp with
  ...
  | Match (arg1, arg2, name1, name2, arg3) ->
    "match " ^ to_string arg1 ^ " with [] -> " ^
    to_string arg2 ^ " | " ^ name1 ^ " :: " ^
    name2 ^ " -> " ^ to_string arg3
  | Raise (arg) ->
    "raise (Error " ^ to_string arg ^ ") "
  | Try (arg1, name, arg2) ->
    "try " ^ to_string arg1 ^ " with Error " ^ name ^ " -> " ^ to_string arg2
